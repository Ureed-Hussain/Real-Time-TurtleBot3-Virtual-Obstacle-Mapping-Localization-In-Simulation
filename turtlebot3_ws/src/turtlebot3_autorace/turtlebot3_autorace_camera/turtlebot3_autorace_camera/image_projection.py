...
class ImageProjection(Node):
    ...
    def __init__(self):
        ...
        parameter_descriptor_640 = ParameterDescriptor(
            description='projection range with limit 640.',
            integer_range=[
                IntegerRange(
                    from_value=0,
                    to_value=640,
        ...
        parameter_descriptor_offset = ParameterDescriptor(
        ...
        self.declare_parameters(
            namespace='',
            parameters=[
                ('camera.extrinsic_camera_calibration.top_x', 72, parameter_descriptor_640),
                ('camera.extrinsic_camera_calibration.top_y', 4, parameter_descriptor_640),
                ('camera.extrinsic_camera_calibration.bottom_x', 259, parameter_descriptor_640),
                ('camera.extrinsic_camera_calibration.bottom_y', 159, parameter_descriptor_640),
                ('camera.extrinsic_camera_calibration.offset_x', 0, parameter_descriptor_offset),
                ('camera.extrinsic_camera_calibration.offset_y', 0, parameter_descriptor_offset),
        ...
        self.offset_x = self.get_parameter(
            'camera.extrinsic_camera_calibration.offset_x').get_parameter_value().integer_value
        self.offset_y = self.get_parameter(
            'camera.extrinsic_camera_calibration.offset_y').get_parameter_value().integer_value
    ...
    def cbGetImageProjectionParam(self, parameters):
        for param in parameters:
            ...
            if param.name == 'camera.extrinsic_camera_calibration.offset_x':
                self.offset_x = param.value
            if param.name == 'camera.extrinsic_camera_calibration.offset_y':
                self.offset_y = param.value
        self.get_logger().info(f'change: top_x={self.top_x}, top_y={self.top_y}')
        self.get_logger().info(f'change: bottom_x={self.bottom_x}, bottom_y={self.bottom_y}')
        self.get_logger().info(f'change: offset_x={self.offset_x}, offset_y={self.offset_y}')
    ...
    def cbImageProjection(self, msg_img):
        ...
        if self.is_calibration_mode:
            ...
            # draw lines to help setting homography variables (with offset applied)
            cv_image_calib = cv2.line(
                cv_image_calib,
                (160 - top_x + offset_x, 180 - top_y + offset_y),
                (160 + top_x + offset_x, 180 - top_y + offset_y),
                (0, 0, 255),
                1
            )
            cv_image_calib = cv2.line(
                cv_image_calib,
                (160 - bottom_x + offset_x, 120 + bottom_y + offset_y),
                (160 + bottom_x + offset_x, 120 + bottom_y + offset_y),
                (0, 0, 255),
                1
            )
        ...
        # selecting 4 points from the original image (with offset applied)
        pts_src = np.array([
            [160 - top_x + offset_x, 180 - top_y + offset_y],
            [160 + top_x + offset_x, 180 - top_y + offset_y],
            [160 + bottom_x + offset_x, 120 + bottom_y + offset_y],
